...
(deftest from-string-test
  ...
  (testing "link"
    ...
    (testing "reference"
      ...
      (testing "no match"
        (are [s] (= (-> s from-string :children first :children)
                    [(node {:tag :txt :content s})])
             "[abc][xyz]"
             "[abc][]"
             "[abc]"))
  ...
  (testing "nested inline"
    (testing "text within emphasis"
      (is (= (-> "*abc*" from-string :children first :children)
             [(branch [{:tag :em} {:tag :txt :content "abc"}])])))

    (testing "deep nesting, broad nesting"
      (testing "2 levels"
        (is (= (-> "*a1 *b1* *b2* a2*" from-string :children first :children)
               [(node {:tag :em}
    ...
    (testing "nested __-strong emphasis"
      (is (= (-> "__abc, __xyz__, pqr__" from-string :children first :children)
             [(node {:tag :strong}
                    [(node {:tag :txt :content "abc, "})
    ...
    (testing "nested *-emphasis"
      (is (= (-> "*(*abc*)* xyz *pqr*" from-string :children first :children)
             [(node {:tag :em}
                    [(node {:tag :txt :content "("})
                     (branch [{:tag :em} {:tag :txt :content "abc"}])
                     (node {:tag :txt :content ")"})])
    ...
    (testing "emphasis delimiter runs: multiples of 3"
      (testing "sum is, parts aren't"
        (are [s c] (= c (-> s from-string :children first :children))
  ...
  (testing "hard line break"
    (testing "spaces at beginning of next line"
      (is (= (-> "abc  \n     xyz" from-string :children first :children)
             [(node {:tag :txt :content "abc"})
    ...
    (testing "inside code span"
      (are [s c] (= (-> s from-string :children first :children)
    ...
    (testing "inside inline link"
      (are [s c] (= (-> s from-string :children first :children first)
...
;          "\r"
  ...
  (testing "post-processing"
    (testing "hard line break at end of block"
      ...
      (testing "(backslash)"
        (are [s cs] (= cs
                       (-> s from-string :children first :children))
             "*abc*\\"    [(branch [{:tag :em} {:tag :txt :content "abc"}])
                           (node {:tag :txt :content "\\"})]
    ...
    (testing "block containing only a hard line break"
      (is (nil? (-> "  " from-string :children first :children))))
  ...
  (testing "backslash escapes"
    ...
    (testing "backslash-escaped backslash escape"
      (are [m t] (= (-> (str \\ \\ m) from-string :children first :children)
                    t)
